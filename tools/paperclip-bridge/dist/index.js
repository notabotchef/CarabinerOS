    ...
    return new Promise((resolve) => {
        ...
        const timer = setTimeout(async () => {
            ...
            try {
                ...
                finish(`Watch timed out after ${timeoutSeconds}s.\n` +
        ...
        ws.on('message', async (raw) => {
            try {
                ...
                if (eventIssueId === issueId && TERMINAL_STATES.includes(newStatus)) {
                    ...
                    const commentList = Array.isArray(comments) ? comments : (comments?.comments ?? comments?.data ?? []);
                    const latest = commentList.length > 0 ? commentList[commentList.length - 1] : null;
                    finish(`Issue transitioned to: ${newStatus}\n` +
...
// Server setup
const server = new Server({ name: 'paperclip-bridge', version: '1.0.0' }, { capabilities: { tools: {} } });
server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
        {
            name: 'delegate',
            description: 'Delegate a task to the Paperclip CEO agent. Creates an issue and assigns it. Use for any work you want the AI team to handle.',
            inputSchema: {
                type: 'object',
                properties: {
                    message: {
        ...
        {
            name: 'status',
            description: 'Check status of a specific issue or get a dashboard of all active work.',
            inputSchema: {
                type: 'object',
                properties: {
                    issueId: {
        ...
        {
            name: 'watch',
            description: 'Watch an issue via WebSocket until it reaches a terminal state (done/blocked/cancelled). Returns the CEO final response.',
            inputSchema: {
                type: 'object',
                properties: {
                    issueId: {
        ...
        {
            name: 'inbox',
            description: 'View the last 10 issues sorted by most recently updated. Quick overview of all work.',
            inputSchema: {
                type: 'object',
                properties: {},
            },
