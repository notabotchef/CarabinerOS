...
class RestaurantContext(Extension):
    async def execute(
        ...
        **kwargs: Any,
        ...
        context_parts.append("")
        context_parts.append("## Write Operations (via carabiner_db MCP)")
        context_parts.append("CRITICAL: For ANY create/add/update/delete operation, YOU must call the carabiner_db MCP tool DIRECTLY.")
        ...
        context_parts.append("These are auto-discovered MCP tools with the prefix 'carabiner_db.'")
        context_parts.append("")
        context_parts.append("Key write tools by domain:")
        context_parts.append("- carabiner_db.inventory_create / inventory_update / inventory_delete")
        context_parts.append("- carabiner_db.orders_create / orders_update / orders_delete")
        context_parts.append("- carabiner_db.prep_create / prep_update / prep_delete")
        context_parts.append("- carabiner_db.invoices_create / invoices_update / invoices_delete")
        context_parts.append("- carabiner_db.recipes_create / recipes_update / recipes_delete")
        context_parts.append("- carabiner_db.menu_create / menu_update / menu_delete")
        context_parts.append("- carabiner_db.food_cost_create / food_cost_update / food_cost_delete")
        context_parts.append("- carabiner_db.campaigns_create / campaigns_update / campaigns_delete")
        context_parts.append("")
        context_parts.append("When the user asks to add, create, update, or delete — call the carabiner_db tool yourself. Never delegate.")
        context_parts.append("")
        context_parts.append("## Action Cards (Direct Notifications)")
        context_parts.append("After ANY database write (create, update, delete), evaluate whether the change warrants chef review.")
        context_parts.append("If YES — call notify_user DIRECTLY with the following fields:")
        context_parts.append("  title   : Kitchen-ticket style, max 80 chars, short and actionable (e.g. 'Produce PO drafted — Coastal, 18 items, cutoff 5pm')")
        context_parts.append("  message : What happened in one sentence")
        context_parts.append("  detail  : 2-3 sentences with specific numbers, costs, quantities")
        context_parts.append("  type    : 'warning' for deadlines/urgent, 'error' for failures, 'success' for routine changes, 'info' for background")
        context_parts.append("")
        context_parts.append("DO create a notification for: new orders, prep list changes, recipe cost changes, inventory alerts, invoice processing, status changes.")
        context_parts.append("DO NOT create a notification for: typo fixes, minor text edits, read-only queries, or when the chef explicitly said 'just do it'.")
        context_parts.append("")
        context_parts.append("NEVER delegate to a subordinate for notifications — call notify_user yourself.")
        context_parts.append("")
