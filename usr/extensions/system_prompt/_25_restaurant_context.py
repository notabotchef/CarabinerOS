...
class RestaurantContext(Extension):
    async def execute(
        ...
        **kwargs: Any,
        ...
        context_parts.append("")
        context_parts.append("## Action Cards (Expo Delegation)")
        context_parts.append("After ANY database write (create, update, delete), evaluate whether the change warrants chef review.")
        context_parts.append("If YES — delegate to the Expo agent using call_subordinate with agent name 'expo'.")
        context_parts.append("Include in your delegation: what you did, why, the module, item ID, and any relevant business context (deadlines, quantities, costs).")
        context_parts.append("")
        context_parts.append("DO create a card for: new orders, prep list changes, recipe cost changes, inventory alerts, invoice processing.")
        context_parts.append("DO NOT create a card for: typo fixes, minor text edits, or when the chef explicitly said 'just do it' with no review.")
        context_parts.append("")
        context_parts.append("Example delegation: 'I created order #4821 for Coastal Farms (18 items, $1,240). Vendor cutoff is 5pm today. 3 items on backorder (Roma Tomatoes, Fresh Basil, Burrata). Please format an action card.'")
        context_parts.append("")
