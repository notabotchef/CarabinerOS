...
class RestaurantContext(Extension):
    async def execute(
        ...
        **kwargs: Any,
        ...
        context_parts.append("For CREATE, UPDATE, or DELETE operations, use the carabiner-db MCP tools.")
        context_parts.append("These are auto-discovered tools with the prefix 'carabiner-db.'")
        context_parts.append("")
        ...
        context_parts.append("- carabiner-db.order_create / order_update / order_delete  (order_tool cannot create)")
        context_parts.append("- carabiner-db.prep_task_create / prep_task_update / prep_task_delete")
        context_parts.append("- carabiner-db.invoice_create / invoice_update / invoice_delete")
        context_parts.append("- carabiner-db.recipe_create / recipe_update / recipe_delete")
        context_parts.append("- carabiner-db.menu_item_create / menu_item_update / menu_item_delete")
        context_parts.append("- carabiner-db.food_cost_create / food_cost_update / food_cost_delete")
        context_parts.append("- carabiner-db.campaign_create / campaign_update / campaign_delete")
        context_parts.append("")
        context_parts.append("When the user asks to add, create, update, edit, or delete any record — use carabiner-db MCP tools.")
