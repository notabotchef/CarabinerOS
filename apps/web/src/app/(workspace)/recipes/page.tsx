...
function RecipeCard({ recipe }: { recipe: Recipe }) {
  return (
    <Link href={`/recipes/${recipe.id}`}>
      <Card className="group relative overflow-hidden border-border/50 bg-card/50 backdrop-blur transition-all duration-200 hover:border-primary/30 hover:bg-card/80 hover:shadow-lg hover:shadow-primary/5 cursor-pointer p-0">
        {/* Image area */}
        <div className="relative h-36 bg-gradient-to-br from-zinc-800/80 to-zinc-900/80 flex items-center justify-center overflow-hidden">
          ...
          <Badge
            ...
            className={`absolute top-3 right-3 text-[10px] ${statusColor(recipe.status)}`}
          >
...
export default function RecipesPage() {
  ...
  return (
    <>
      <WorkspaceHeader title="Recipes" subtitle="Modernist Cuisine recipe system" />
      <div className="p-4 lg:p-6 space-y-6">
        {/* KPI row */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {(["all", "active", "draft", "archived"] as const).map((key) => (
            <Card
              key={key}
              className={`p-3 cursor-pointer transition-all border-border/50 hover:border-primary/30 ${
                statusFilter === key ? "border-primary/50 bg-primary/5" : "bg-card/50"
              }`}
